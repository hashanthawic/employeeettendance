<?php 
/**
 * Employee_attendance Page Controller
 * @category  Controller
 */
class Employee_attendanceController extends BaseController{
	/**
     * Load Record Action 
     * $arg1 Field Name
     * $arg2 Field Value 
	...
	function index($fieldname = null , $fieldvalue = null){
		$db = $this->GetModel();
		$tablename = $this->tablename = 'employee_attendance';
		$fields = array('id', 
			'Name', 
			'Username', 
			'Password', 
		...
		if(!empty($this->search)){
			...
			$db->where("(id LIKE ? OR Name LIKE ? OR Username LIKE ? OR Password LIKE ? OR Registrationdate LIKE ? OR Contact_Number LIKE ? OR Contactnumber LIKE ?)", array("%$text%","%$text%","%$text%","%$text%","%$text%","%$text%","%$text%"));
		}
		if(!empty($this->orderby)){ // when order by request fields (from $_GET param)
			$db->orderBy($this->orderby,$this->ordertype);
		}
		else{
			$db->orderBy('employee_attendance.id', ORDER_TYPE);
		}
		if( !empty($fieldname) ){
		...
		$this->view->page_title ="Employee Attendance";
		$this->view->render('employee_attendance/list.php' , $data ,'main_layout.php');
	}
	/**
     * View Record Action 
     * @return View
	...
	function view( $rec_id = null , $value = null){
		...
		if( !empty($value) ){
			...
			$db->where('employee_attendance.id' , $rec_id);
		}
		$record = $db->getOne($tablename, $fields );
		if(!empty($record)){
			$this->view->page_title ="View  Employee Attendance";
			$this->view->render('employee_attendance/view.php' , $record ,'main_layout.php');
		}
		else{
			$page_error = null;
			if($db->getLastError()){
	...
	function add(){
				...
				if(!empty($rec_id)){
					if(is_ajax()){
						...
						if(!empty($this->redirect)){ 
							...
							redirect_to_page("employee_attendance");
						}
					}
					return;
				}
		...
		$this->view->page_title ="Add New Employee Attendance";
		$this->view->render('employee_attendance/add.php' ,null,'main_layout.php');
	}
	/**
     * Edit Record Action 
	...
	function edit($rec_id = null){
		...
		$this->view->page_title ="Edit  Employee Attendance";
		if(!empty($data)){
			$this->view->render('employee_attendance/edit.php' , $data, 'main_layout.php');
		}
		else{
			if($db->getLastError()){
	...
	function delete( $rec_ids = null ){
		...
		foreach($arr_id as $rec_id){
			$db->where('employee_attendance.id' , $rec_id,"=",'OR');
		}
