<?php 
/**
 * Checkin Page Controller
 * @category  Controller
 */
class CheckinController extends SecureController{
	/**
     * Load Record Action 
     * $arg1 Field Name
     * $arg2 Field Value 
	...
	function index($fieldname = null , $fieldvalue = null){
		$db = $this->GetModel();
		$tablename = $this->tablename = 'checkin';
		$fields = array('id', 
			'user', 
			'checkin');
		...
		if(!empty($this->search)){
			...
			$db->where("(id LIKE ? OR user LIKE ? OR checkin LIKE ?)", array("%$text%","%$text%","%$text%"));
		}
		if(!empty($this->orderby)){ // when order by request fields (from $_GET param)
			$db->orderBy($this->orderby,$this->ordertype);
		}
		else{
			$db->orderBy('checkin.id', ORDER_TYPE);
		}
		if( !empty($fieldname) ){
		...
		$this->view->page_title ="Checkin";
		$this->view->render('checkin/list.php' , $data ,'main_layout.php');
	}
	/**
     * View Record Action 
     * @return View
	...
	function view( $rec_id = null , $value = null){
		...
		if( !empty($value) ){
			...
			$db->where('checkin.id' , $rec_id);
		}
		$record = $db->getOne($tablename, $fields );
		if(!empty($record)){
			$this->write_to_log('view', 'true');
			$this->view->page_title ="View  Checkin";
			$this->view->render('checkin/view.php' , $record ,'main_layout.php');
		}
		else{
			$page_error = null;
			if($db->getLastError()){
	...
	function add(){
		if(is_post_request()){
			...
			$db = $this->GetModel();
			$tablename = $this->tablename = 'checkin';
			$fields = $this->fields = array('user','checkin'); //insert fields
		...
		$this->view->page_title ="Add New Checkin";
		$this->view->render('checkin/add.php' ,null,'main_layout.php');
	}
	/**
     * Edit Record Action 
	...
	function edit($rec_id = null){
		...
		$this->rec_id = $rec_id;
		$tablename = $this->tablename = 'checkin';
		$fields = $this->fields = array('id','user','checkin'); //editable fields
		if(is_post_request()){
			Csrf :: cross_check();
			$postdata = $this->transform_request_data($_POST);
			$this->rules_array = array(
		...
		$this->view->page_title ="Edit  Checkin";
		if(!empty($data)){
			$this->view->render('checkin/edit.php' , $data, 'main_layout.php');
		}
		else{
			if($db->getLastError()){
