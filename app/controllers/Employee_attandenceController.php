<?php 
/**
 * Employee_attandence Page Controller
 * @category  Controller
 */
class Employee_attandenceController extends SecureController{
	/**
     * Load Record Action 
     * $arg1 Field Name
     * $arg2 Field Value 
	...
	function index($fieldname = null , $fieldvalue = null){
		$db = $this->GetModel();
		$tablename = $this->tablename = 'employee attandence';
		$fields = array('id', 
			'Name', 
			'Username', 
			'Registrationdate', 
		...
		if(!empty($this->orderby)){ // when order by request fields (from $_GET param)
			...
			$db->orderBy('employee attandence.id', ORDER_TYPE);
		}
		if( !empty($fieldname) ){
		...
		$this->view->page_title ="Employee Attandence";
		$this->view->render('employee_attandence/list.php' , $data ,'main_layout.php');
	}
	/**
     * View Record Action 
     * @return View
	...
	function view( $rec_id = null , $value = null){
		...
		if( !empty($value) ){
			...
			$db->where('employee attandence.id' , $rec_id);
		}
		$record = $db->getOne($tablename, $fields );
		if(!empty($record)){
			$this->view->page_title ="View  Employee Attandence";
			$this->view->render('employee_attandence/view.php' , $record ,'main_layout.php');
		}
		else{
			$page_error = null;
			if($db->getLastError()){
	...
	function add(){
		if(is_post_request()){
			...
			$this->rules_array = array(
				...
				'Contactnumber' => 'required|valid_email',
			);
				...
				if(!empty($rec_id)){
					if(is_ajax()){
						...
						if(!empty($this->redirect)){ 
							...
							redirect_to_page("employee_attandence");
						}
					}
					return;
				}
		...
		$this->view->page_title ="Add New Employee Attandence";
		$this->view->render('employee_attandence/add.php' ,null,'main_layout.php');
	}
	/**
     * Edit Record Action 
	...
	function edit($rec_id = null){
		...
		$this->rec_id = $rec_id;
		$tablename = $this->tablename = 'employee attandence';
		$fields = $this->fields = array('id','Name','Username','Registrationdate'); //editable fields
		if(is_post_request()){
			Csrf :: cross_check();
			$postdata = $this->transform_request_data($_POST);
			$this->rules_array = array(
		...
		$this->view->page_title ="Edit  Employee Attandence";
		if(!empty($data)){
			$this->view->render('employee_attandence/edit.php' , $data, 'main_layout.php');
		}
		else{
			if($db->getLastError()){
	...
	function delete( $rec_ids = null ){
		...
		foreach($arr_id as $rec_id){
			$db->where('employee attandence.id' , $rec_id,"=",'OR');
		}
		...
		if(!empty($this->redirect)){ 
			...
			redirect_to_page("employee attandence");
		}
	}
}
