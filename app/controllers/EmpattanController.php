<?php 
/**
 * Empattan Page Controller
 * @category  Controller
 */
class EmpattanController extends BaseController{
	/**
     * Load Record Action 
     * $arg1 Field Name
     * $arg2 Field Value 
	...
	function index($fieldname = null , $fieldvalue = null){
		$db = $this->GetModel();
		$tablename = $this->tablename = 'empattan';
		$fields = array('id', 
			'Name', 
			'Username', 
			'Password', 
		...
		if(!empty($this->search)){
			...
			$db->where("(id LIKE ? OR Name LIKE ? OR Username LIKE ? OR Password LIKE ? OR Registration_Date LIKE ? OR Contact_Number LIKE ?)", array("%$text%","%$text%","%$text%","%$text%","%$text%","%$text%"));
		}
		if(!empty($this->orderby)){ // when order by request fields (from $_GET param)
			$db->orderBy($this->orderby,$this->ordertype);
		}
		else{
			$db->orderBy('empattan.id', ORDER_TYPE);
		}
		if( !empty($fieldname) ){
		...
		$this->view->page_title ="Empattan";
		$this->view->render('empattan/list.php' , $data ,'main_layout.php');
	}
	/**
     * View Record Action 
     * @return View
	...
	function view( $rec_id = null , $value = null){
		...
		if( !empty($value) ){
			...
			$db->where('empattan.id' , $rec_id);
		}
		$record = $db->getOne($tablename, $fields );
		if(!empty($record)){
			$this->view->page_title ="View  Empattan";
			$this->view->render('empattan/view.php' , $record ,'main_layout.php');
		}
		else{
			$page_error = null;
			if($db->getLastError()){
	...
	function add(){
		if(is_post_request()){
			...
			$db = $this->GetModel();
			$tablename = $this->tablename = 'empattan';
			$fields = $this->fields = array('Name','Username','Password','Registration_Date','Contact_Number'); //insert fields
		...
		$this->view->page_title ="Add New Empattan";
		$this->view->render('empattan/add.php' ,null,'main_layout.php');
	}
	/**
     * Edit Record Action 
	...
	function edit($rec_id = null){
		...
		$this->rec_id = $rec_id;
		$tablename = $this->tablename = 'empattan';
		$fields = $this->fields = array('id','Name','Username','Password','Registration_Date','Contact_Number'); //editable fields
		if(is_post_request()){
			Csrf :: cross_check();
			$postdata = $this->transform_request_data($_POST);
			$cpassword = $postdata['confirm_password'];
		...
		$this->view->page_title ="Edit  Empattan";
		if(!empty($data)){
			$this->view->render('empattan/edit.php' , $data, 'main_layout.php');
		}
		else{
			if($db->getLastError()){
	...
	function delete( $rec_ids = null ){
		...
		foreach($arr_id as $rec_id){
			$db->where('empattan.id' , $rec_id,"=",'OR');
		}
