<?php 
/**
 * Checkincheckout Page Controller
 * @category  Controller
 */
class CheckincheckoutController extends SecureController{
	/**
     * Load Record Action 
     * $arg1 Field Name
     * $arg2 Field Value 
	...
	function index($fieldname = null , $fieldvalue = null){
		$db = $this->GetModel();
		$tablename = $this->tablename = 'checkincheckout';
		$fields = array('id', 
			'Checkin', 
			'Checkout');
		...
		if(!empty($this->search)){
			...
			$db->where("(id LIKE ? OR Checkin LIKE ? OR Checkout LIKE ?)", array("%$text%","%$text%","%$text%"));
		}
		if(!empty($this->orderby)){ // when order by request fields (from $_GET param)
			$db->orderBy($this->orderby,$this->ordertype);
		}
		else{
			$db->orderBy('checkincheckout.id', ORDER_TYPE);
		}
		if( !empty($fieldname) ){
		...
		$this->view->page_title ="Checkincheckout";
		$this->view->render('checkincheckout/list.php' , $data ,'main_layout.php');
	}
	/**
     * View Record Action 
     * @return View
	...
	function view( $rec_id = null , $value = null){
		...
		if( !empty($value) ){
			...
			$db->where('checkincheckout.id' , $rec_id);
		}
		$record = $db->getOne($tablename, $fields );
		if(!empty($record)){
			$this->view->page_title ="View  Checkincheckout";
			$this->view->render('checkincheckout/view.php' , $record ,'main_layout.php');
		}
		else{
			$page_error = null;
			if($db->getLastError()){
	...
	function add(){
		if(is_post_request()){
			...
			$db = $this->GetModel();
			$tablename = $this->tablename = 'checkincheckout';
			$fields = $this->fields = array('Checkin','Checkout'); //insert fields
				...
				if(!empty($rec_id)){
					if(is_ajax()){
						...
						if(!empty($this->redirect)){ 
							...
							redirect_to_page("checkincheckout");
						}
					}
					return;
				}
		...
		$this->view->page_title ="Add New Checkincheckout";
		$this->view->render('checkincheckout/add.php' ,null,'main_layout.php');
	}
	/**
     * Edit Record Action 
	...
	function edit($rec_id = null){
		...
		$this->rec_id = $rec_id;
		$tablename = $this->tablename = 'checkincheckout';
		$fields = $this->fields = array('id','Checkin','Checkout'); //editable fields
		if(is_post_request()){
			Csrf :: cross_check();
			$postdata = $this->transform_request_data($_POST);
			$this->rules_array = array(
		...
		$this->view->page_title ="Edit  Checkincheckout";
		if(!empty($data)){
			$this->view->render('checkincheckout/edit.php' , $data, 'main_layout.php');
		}
		else{
			if($db->getLastError()){
	...
	function delete( $rec_ids = null ){
		...
		foreach($arr_id as $rec_id){
			$db->where('checkincheckout.id' , $rec_id,"=",'OR');
		}
