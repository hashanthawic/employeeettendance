...
class PasswordmanagerController extends BaseController{
	...
	function postresetlink(){
		...
		if(!empty($_POST['email'])){
			...
			$db = $this->GetModel();
			//get user by email
			$db->where ('Contactnumber', $email);
			$user = $db->getOne('empattendance');
			if(!empty($user)){
				$password_reset_key=random_str();
				$user_id = $user['id'];
				//Update user password reset key with new one
				$db->where ('id', $user_id);
				$db->update( 'empattendance', array("password_reset_key"=>$password_reset_key) );
	...
	function updatepassword($password_key=null){
		...
		if(!empty($_POST['password'])){
			...
			if($password == $cpassword){
				...
				$db->update('empattendance',$new_password_data);
				if($db->getRowCount()){
